  ...
  return (
    ...
    >
      {(videoStatus === 'pending' || videoStatus === 'processing') && createAt && (
        <VideoProcessing createAt={createAt} />
      )}
      ...
      {(!videoStatus || videoStatus === 'completed') && (
        ...
        >
          ...
          <video
            ...
            preload='none'
            className={cn(
              'absolute inset-0 z-20 h-auto w-full object-contain',
      ...
      <div className='flex flex-col gap-2'>
        ...
        {showBtns && videoUrl && (
          <div className='flex items-center justify-end gap-3'>
            <VideoExtendBtn text={t('extend')} videoUrl={videoUrl} videoId={id} />
            <button
              type='button'
              onClick={handleDownload}
              className='flex h-7 items-center justify-center gap-1 rounded border border-main-gray bg-black px-3 text-sm text-white/40'
            >
...
VideoCardWithoutLink.displayName = 'VideoCardWithoutLink';

export default memo(VideoCardWithoutLink);
