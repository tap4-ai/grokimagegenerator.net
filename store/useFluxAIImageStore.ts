...
export const STORE_NAME = 'flux-ai-image-store';
...
export const useFluxAIImageStore = create(
  persist<FluxAIImageStore>(
    (set) => ({
      prompt: '',
      aspectRatio: {
        width: 16,
...
export interface FluxAIImageStoreForGenerator extends Omit<FluxAIImageStore, 'emptyHistory' | 'removeHistory'> {
  storeName: string;
}

export const useFluxAIImageStoreForGenerator = (): FluxAIImageStoreForGenerator => {
  const storeData = useFluxAIImageStore(
    useShallow((state) => ({
      prompt: state.prompt,
